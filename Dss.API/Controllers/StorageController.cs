...
public class StorageController : ControllerBase
{
    ...
    [DisableRequestSizeLimit]
    [RequestFormLimits(ValueLengthLimit = int.MaxValue, MultipartBodyLengthLimit = int.MaxValue)]
    [HttpPost(nameof(UploadFileInDatalakeWithSASUrl))]
    public async Task<IActionResult> UploadFileInDatalakeWithSASUrl(IFormFile file, string sasUri)
    {
        var timer = new Stopwatch();
        timer.Start();

        BlobResponseDto? response = await _storage.UploadFileInDatalakeWithSASUrlAsync(file, sasUri);
