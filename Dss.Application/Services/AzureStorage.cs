...
public class AzureStorage : IAzureStorage
{
    ...
    public async Task<BlobResponseDto?> UploadFileInDatalakeWithSASUrlAsync(IFormFile blob, string sasUri)
    {
        // Create new upload response object that we can return to the requesting method
        BlobResponseDto response = new();
        try
        {
            Uri conUri = new(sasUri);           
            DataLakeDirectoryClient directoryClient = new (conUri);
            DataLakeFileClient fileClient = directoryClient.GetFileClient(blob.FileName);

            // Open a stream for the file we want to upload
            await using Stream? data = blob.OpenReadStream();
            // Upload the file async
            await fileClient.UploadAsync(data, overwrite: true);


            // Everything is OK and file got uploaded
            response.Status = $"File {blob.FileName} Uploaded Successfully";
            response.Error = false;
            response.Blob.Uri = fileClient.Uri.AbsoluteUri;
            response.Blob.Name = fileClient.Name;
